      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-5">
        <div class="space-y-1">
          ...
          <h1 class="text-2xl font-extrabold tracking-tight text-neutral-900 dark:text-neutral-50 flex items-center gap-2">
            <FolderPlus class="h-6 w-6 text-indigo-500" />
            <span>Create Portfolio Project</span>
          </h1>
          <p class="text-xs text-muted-foreground">Configure project metadata, category tags, tech stack chips, media URLs, and showcase positioning.</p>
        </div>
...
const techChips = ref(['Laravel 11', 'Vue 3', 'Tailwind CSS']);
  ...
  form.post(route('admin.projects.store'), {
    onSuccess: () => {
      toast({
        ...
        description: `"${form.title}" added to your portfolio successfully.`,
        type: 'success'
      });
    },
  });
